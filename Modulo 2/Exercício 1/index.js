...
const total = json.students.length

let infosPerState = [{ }]

let stateCount = getStates(json.students).length

function getStates(arr) {
    let states = []

    arr.map((item) => {
        states.includes(item.state)  ? null : states.push(item.state)
    })
...
getStates(json.students).map(( item ) => {
    infosPerState[0][item] =  {
        "totalStudents": 0,
        "averageAge": 0,
    }
})

for (const key in infosPerState[0]) {
    json.students.map(( item ) => {
        if(item.state == key){
            infosPerState[0][key]['totalStudents']++
            infosPerState[0][key]['averageAge'] += parseInt(item.age)
        }
    })

    let average = infosPerState[0][key]['averageAge'] / infosPerState[0][key]['totalStudents']

    infosPerState[0][key]['averageAge'] = parseInt(average.toFixed(0))
}

console.log('Total: ' + total)
console.log('StateCount: ' + stateCount)
console.log('infosPerState: ' , infosPerState)
